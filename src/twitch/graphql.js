...
const GraphQL = {
  ...
  SendQuery: async (
    ...
    if (!GraphQL.ClientID) throw "Please make sure to fill in a ClientID";

    if (!preset) {
      body.query = fs.readFileSync(
        `${__dirname}/../queries/${QueryName}.gql`,
        "UTF-8"
      );
    } else {
    ...
    if (Integrity) {
      ...
      cookies.forEach((cookie) => {
        if (cookie.startsWith("server_session_id")) {
          ...
          cookie.startsWith("unique_id") &&
          !cookie.startsWith("unique_id_durable")
        ) {
          let value = cookie.match(/(?<=\=)\w+(?=\;)/g) || [];
          deviceid = value[0];
        }
    ...
    if (
      GraphGQLResponse.errors ||
      (GraphGQLResponse[0] && GraphGQLResponse[0].errors) ||
      GraphGQLResponse.error
    ) {
      return await errorHandler(
        GraphGQLResponse,
        QueryName,
...
async function errorHandler(
  ...
  if (GraphQL.retries < GraphQL.maxretries) {
    ...
    return await GraphQL.SendQuery(
      QueryName,
      variables,
      sha256Hash,
      OAuth,
