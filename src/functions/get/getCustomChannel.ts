...
export async function getCustomChannel() {
  ...
  if (!userdata.settings.displayless) {
    if (fs.existsSync(path)) {
      ...
      winston.info(
        chalk.gray(
          "Found " + userdata.customchannel.length + " Custom Channels..."
        )
      );
      winston.silly(" ");
    ...
    if (fs.existsSync(path)) {
      ...
      winston.info(
        chalk.gray(
          "Closing Bot, somehow there is no Customchannels file anymore...!"
        )
      );
      if (!userdata.settings.displayless)
        inputReader.wait(chalk.gray("Press any Key to continue..."));
...
export async function askCustomChannelStart(
  random: boolean,
  filterlive: boolean
) {
      ...
      .then(async (answer: { namelist: string }) => {
        userdata.customchannel.forEach((drop) => {
          if (drop.Name === answer.namelist) {
            userdata.startDrop = drop.TTVLink.split(
              "https://www.twitch.tv/"
            )[1];
          }
        });
    ...
    .then(
      async (answers: {
        ...
        await fs.promises
          .writeFile(
            "./CustomChannels.json",
            JSON.stringify(userdata.customchannel, null, 2)
          )
          .then(function () {
            winston.silly(" ");
...
export async function customCheckLive(feedback: boolean) {
  for (const customchannel of userdata.customchannel) {
    ...
    if (feedback) {
      winston.silly(" ");
      winston.info(
        chalk.cyan(customchannel.TTVLink) +
          " | " +
          chalk.magenta(customchannel.Name) +
          " | " +
          statustoString(customchannel.live),
        { event: "getResult" }
      );
    }
  }
