...
export async function getTwitchDrops(game: string, feedback: boolean) {
  ...
  const DropCampaignDetails = await TwitchGQL._SendQuery(
  ...
  if (userdata.settings.debug)
    winston.info(
      "DropCampain %o",
      JSON.stringify(DropCampaignDetails, null, 2)
    );
  ...
  for (const e of dropidstoget) {
    ...
    // _SendQuery(QueryName,variables,sha256Hash = null,OAuth = null,preset = false,headers = {},Integrity = false)
    const DropDetails = await TwitchGQL._SendQuery(
      "DropCampaignDetails",
      opts,
  ...
  const rawInventory = await TwitchGQL._SendQuery(
    "Inventory",
    {},
  ...
  if (userdata.settings.debug)
    winston.info("rawinventory %o", JSON.stringify(rawInventory, null, 2));
  ...
  userdata.drops.forEach((DropElement) => {
    if (Inventory.dropCampaignsInProgress !== null) {
      Inventory.dropCampaignsInProgress.forEach(
        (e: DropCampaignsInProgress) => {
  ...
  userdata.drops.forEach((drop) => {
    drop.timebasedrop.forEach((time: any) => {
      if (!("self" in time)) {
        time["self"] = {
  ...
  if (feedback) {
    userdata.drops.forEach((drop) => {
      winston.silly(" ");
      winston.info(
        livechresponse(drop.foundlivech) +
          " | " +
          chalk.magenta(drop.dropname) +
          " | " +
          statustoString(drop.live) +
          " | " +
          claimedstatustoString(drop.isClaimed),
        { event: "getResult" }
      );
    });
  }
...
export async function askWhatDropToStart(
  random: boolean,
  filterlive: boolean,
      ...
      if (userdata.nonActiveDrops.includes(DropName)) {
        ...
        winston.info(
          chalk.yellow(
            DropName +
              " | " +
              "was removed because the drop ended or not started yet..."
          )
        );
      }
    }
  ...
  if (userdata.availableDropNameChoices.length === 0) {
    ...
    winston.info(
      chalk.gray(
        "All available Channels Offline... Select any Drop to start watching..."
      )
    );
    userdata.drops.forEach((drop) => {
      userdata.availableDropNameChoices.push(drop.dropname);
  ...
  if (!random) {
    ...
    userdata.startDrop =
      userdata.availableDropNameChoices[
        getRandomInt(userdata.availableDropNameChoices.length)
      ];
    winston.info(
      chalk.gray(
        "Selected a random drop to watch: " + chalk.white(userdata.startDrop)
      )
    );
  }
}
...
export async function askWhatGameToWatch(random: boolean) {
  ...
  if (!userdata.settings.displayless) {
    if (!random) {
      ...
      winston.info(
        chalk.gray(
          "Selected a random game to watch: " + chalk.white(userdata.game)
        )
      );
    }
  } else {
    if (userdata.settings.Prioritylist.length === 0) {
      ...
      winston.info(
        chalk.gray(
          "Selected a Game from your Priority List watch: " + userdata.game
        )
      );
    }
  }
    ...
    for (let i = userdata.settings.Prioritylist.length; i--; ) {
      if (!activecampainnames.includes(userdata.settings.Prioritylist[i])) {
        winston.info(
          chalk.yellow(
            "Removed " +
              userdata.settings.Prioritylist[i] +
              " from the Priority List, because there is no ACTIVE campaign with such name."
          )
        );
