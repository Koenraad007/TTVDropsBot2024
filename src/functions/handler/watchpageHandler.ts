...
const TwitchGQL = require("../../twitch/twitchgql").Init(userdata.clientid);
...
async function loop(DropcurrentlyWatching: string) {
  ...
  if (userdata.settings.debug)
    winston.info("UserDATA %o", JSON.stringify(userdata, null, 2));
  //Update Drop Data
  await getTwitchDrops(userdata.game, false);
  await allOfflineCheck();
  //Get the right Drop
  if (status === "running") {
    await getCurrentDrop().then(async (CurrentDrop) => {
      if (userdata.settings.debug)
        winston.info("CurrentDrop %o", JSON.stringify(CurrentDrop, null, 2));
      //Switch DropcurrentlyWatching to a live one if current offline and new live ch available
      if (
        !CurrentDrop.foundlivech.includes(DropcurrentlyWatching) &&
        CurrentDrop.foundlivech.length > 0
      ) {
        DropcurrentlyWatching = CurrentDrop.foundlivech[0];
        winston.info(
          chalk.gray(
            "Switched current Channel to " +
              chalk.white(DropcurrentlyWatching) +
              "..."
          )
        );
        winston.silly(" ");
          ...
          chalk.gray("Current Progress: ") +
            ...
            chalk.gray(
              " | Watched " +
                ...
                " Minutes" +
                chalk.gray(" | Drop ") +
                chalk.white(i + 1 + "/" + dropslenght) +
                chalk.gray(" | Status ") +
                chalk.white(drop.self.status) +
                chalk.gray(" | isClaimed ") +
                chalk.white(drop.self.isClaimed)
            ),
          { event: "progress" }
        );
      }
...
export async function sendMinuteWatched(ChannelLogin: string) {
  ...
  let Stream = await TwitchGQL._SendQuery(
    "UseLive",
    opts,
  ...
  let SettingsJSReg = new RegExp(
    "https://static.twitchcdn.net/config/settings.[0-9a-f]{32}.js"
  );
  let parsehtml = SettingsJSReg.exec(gethtml.data.toString());
  if (parsehtml![0] === null)
    winston.error("Error while parsing Settings Url...");

  const getSettingsJS = await axios
    .get(parsehtml![0].toString(), { raxConfig: retryConfig })
    .catch((err) => {
      winston.error(
        "ERROR: Could not load your twitch settings... Check your connection..."
      );
      throw err;
    });
  ...
  const post = await axios
    .post(parseJS![0].toString(), b64, config)
    .catch((err) => {
