...
export async function liveCheck(channelLogin: string, custom: boolean) {
  if (channelLogin !== undefined) {
    ...
    if (!status) {
      winston.info(
        chalk.red("Current Channel offline... Looking for new one..."),
        { event: "offline" }
      );
      if (custom) {
    ...
    winston.info(
      chalk.red(
        "No Channel Live at the moment for this Drop... Looking for new one..."
      ),
      { event: "offline" }
    );
    if (custom) {
...
export async function allOfflineCheck() {
  ...
  if (dropsoffline === userdata.drops.length) {
    if (
      userdata.settings.WaitforChannels &&
      userdata.settings.Prioritylist.length === 0
    ) {
      winston.silly(" ");
      winston.info(
        chalk.red(
          "All Drops for this game are offline... Looking for new live Channels in 5 minutes..."
        ),
        { event: "offline" }
      );
      ...
      winston.info(
        chalk.red(
          "All Drops for this game are offline... Looking for new game..."
        ),
        { event: "newGame" }
      );
...
export async function customallOfflineCheck() {
  ...
  if (dropsoffline === userdata.customchannel.length) {
    ...
    winston.info(
      chalk.red(
        "All Channels are offline... Looking for new live Channels in 5 minutes..."
      ),
      { event: "offline" }
    );
    winston.silly(" ", { event: "progressEnd" });
