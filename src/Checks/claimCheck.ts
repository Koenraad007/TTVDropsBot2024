...
const TwitchGQL = require("../twitch/twitchgql").Init();

export async function claimableCheck(
  CurrentDrop: Drop,
  autoclaim: boolean,
  onlycheck: boolean
) {
  ...
  CurrentDrop.timebasedrop.forEach((timedrop) => {
    if (
      (!timedrop.self.isClaimed && timedrop.self.status === "Not Active") ||
      (!timedrop.self.isClaimed && timedrop.self.status === "Ended")
    ) {
      nonworkingamount++;
    }
  ...
  for (const timedrop of CurrentDrop.timebasedrop) {
    if (
      timedrop.requiredMinutesWatched === timedrop.self.currentMinutesWatched
    ) {
      hundredpercent++;
    }
      ...
      for (const benefit of timedrop.benefitEdges) {
        if (
          timedrop.self.currentMinutesWatched ===
            timedrop.requiredMinutesWatched &&
          timedrop.self.dropInstanceID !== null
        ) {
          let opts = {
            input: {
          ...
          try {
            ...
            if (autoclaim)
              winston.info(
                chalk.gray("Claimed " + chalk.green(timedrop.name)),
                { event: "claim" }
              );
            if (preconditions && !autoclaim)
              winston.info(
                chalk.gray(
                  "Claimed " +
                    chalk.green(timedrop.name) +
                    " because otherwise cant watch next drop..."
                ),
                { event: "claim" }
              );
          } catch (e) {
            if (autoclaim)
              winston.info(
                chalk.gray(
                  "There was an error trying to claim " +
                    chalk.green(timedrop.name) +
                    " Will retry again."
                ),
                { event: "claim" }
  ...
  //All Claimable
  if (
    workingdropslenght !== CurrentDrop.timebasedrop.length &&
    notavaiableyet >= isclaimedamount + hundredpercent
  ) {
    if (!onlycheck) {
      winston.silly(" ");
      winston.info(
        chalk.green(
          "Got all available Drops, missing Drops are not active yet... Looking for new ones..."
        ),
        { event: "newDrop" }
      );
    ...
    if (!onlycheck) {
      ...
      winston.info(
        chalk.green(
          "All available Drops for Current Drop are unavailable... Looking for new ones..."
        ),
        { event: "newDrop" }
      );
      await restartHandler(true, true, true, true, false);
    ...
    if (!onlycheck) {
      ...
      winston.info(
        chalk.green(
          "All available Drops for Current Drop Claimable... Looking for new ones..."
        ),
        { event: "newDrop" }
      );
      await restartHandler(true, true, true, true, false);
    ...
    if (!onlycheck) {
      ...
      winston.info(
        chalk.green(
          "All Drops for Current Drop Claimed... Looking for new ones..."
        ),
        { event: "newDrop" }
      );
      await restartHandler(true, true, true, true, false);
    ...
    if (!onlycheck) {
      ...
      winston.info(
        chalk.green(
          "All available Drops for Current Drop Claimable or Claimed... Looking for new ones..."
        ),
        { event: "newDrop" }
      );
      await restartHandler(true, true, true, true, false);
...
async function allgameddropsclaimableCheck() {
  ...
  if (isclaimedorclaimableamount >= amount - nonworkingamount) {
    ...
    if (userdata.settings.Prioritylist.length === 0)
      winston.warn(
        chalk.yellow(
          "Warning: Please add Games to your Priority List, otherwise the bot will select a random game... or disable this feature in the settings... or disable this feature in the settings..."
        )
      );
    winston.info(
      chalk.green(
        "All available drops of the game claimed or claimable... Looking for a new Game...."
      ),
      { event: "newGame" }
    );
    ...
    isclaimedorclaimableamount >=
    amount - nonworkingamount - offlinedrops
  ) {
    winston.silly(" ");
    if (userdata.settings.WaitforChannels) {
      winston.info(
        chalk.green(
          "All available Live Drops of the game claimed or claimable... Looking for new Live Drop in 5 Minutes...."
        ),
        { event: "newDrop" }
      );
      ...
      if (userdata.settings.Prioritylist.length === 0)
        winston.warn(
          chalk.yellow(
            "Warning: Please add Games to your Priority List, otherwise the bot will select a random game... or disable this feature in the settings..."
          )
        );
      winston.info(
        chalk.green(
          "All available Live Drops of the game claimed or claimable... Looking for a new Game...."
        ),
        { event: "newGame" }
      );
      await restartHandler(true, true, true, true, true);
    ...
    userdata.drops.forEach((drop) => {
      drop.timebasedrop.forEach((timebasedrop) => {
        timebasedrop.benefitEdges.forEach((benefit) => {
          if (
            claimeddrop.imageurl.toString() ===
            benefit.benefit.imageAssetURL.toString()
          ) {
