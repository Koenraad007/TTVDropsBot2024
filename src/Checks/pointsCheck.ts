...
export async function pointsCheck(channelLogin: string) {
  ...
  const pointsrequest = await TwitchGQL._SendQuery(
...
async function checkisClaimeable(
  request: any,
  channelId: string,
  autopoints: boolean
) {
  ...
  try {
    ClaimId =
      request[0].data.community.channel.self.communityPoints.availableClaim.id;
  } catch (e) {
  ...
  if (ClaimId !== "") {
    ...
    if (autopoints) {
      ...
      const claimrequest = await TwitchGQL._SendQuery(
